def run(filename):
    ...
    times_total = [int(time.strip()) for time in times_total.split(' ') if time.strip()]

    distance_records = lines[1].split(':')[1].strip()
    distance_records = [int(distance.strip()) for distance in distance_records.split(' ') if distance.strip()]
